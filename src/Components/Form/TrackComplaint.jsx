...
const TrackComplaint = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      const res = await axios.get(`http://localhost:4000/track_complaint/${complaintId}`);
            ...
            <div>
              ...
              <ul className="list-disc ml-6">
                {complaintData.evidence_files.map((file, index) => (
                  <li key={index}>
                    <a href={`http://localhost:4000/${file}`} target="_blank" rel="noopener noreferrer" className="text-blue-600 underline">
                      {file.split("\\").pop()}
                    </a>
                  </li>
                ))}
